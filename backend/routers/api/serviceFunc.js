...
const getDeleteStr = async (req, db, userId, key) => {
  let entries = await req.conn.queryAsync(
    `SELECT id FROM ? WHERE user_fk = ${id}`,
    [db]
  );
  let entriesStr = "";
...
const getBMR = async (req, bodyweight, height, age, gender) => {
  ...
  let genderNum = await req.conn.queryAsync(
    `SELECT bmr_num FROM gender WHERE id = ${gender}`
  );
...
const updateMaintenanceCal = async (req, userId, updateDate, tz) => {
  let userArr = await req.conn.queryAsync(
  ...
  if (latestBodyweight.length === 0)
    throw Error("User does not have recorded bodyweight");

  let deletePacket = await req.conn.queryAsync(
    `DELETE FROM maintenance_calories WHERE user_fk = ${user.id} AND date = '${updateDate}'`
  );

  let kgBodyweight = convertUnit(
    latestBodyweight[0].weight,
    user.bw_unit_fk,
    1
  );
...
const updateLiftMax = async (req, liftId) => {
  ...
  if (max.length === 0 || theomax.length === 0)
    throw Error("Lift has no sets yet.");
...
const checkExistingLiftSet = async (req, liftId, date) => {
  let dateSet = await req.conn.queryAsync(
    `SELECT * FROM lift_set WHERE date = '${date}' AND lift_fk = ${liftId}`
  );
  return dateSet;
};
